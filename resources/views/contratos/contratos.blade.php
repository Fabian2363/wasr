...
<div class="pull-left breadcrumb_admin clear_both">
    <div class="pull-left page_title theme_color">
        <h1>@lang('Información de Contratos ')</h1>
        <h2 class="">@lang('')</h2>
    </div>
        ...
        <div class="ContenedorFormularioCenso">
            <div class="color_infor noPrint" style="margin-top: 15px;">
                <span class="color_infor  noPrint">@lang('Usted se encuentra en:')&nbsp;&nbsp;@lang('Contratos Laborales') &gt; <font
                        color="#666666">@lang('Información de Contratos ') </font></span>
            </div>
            ...
            <div class="block-web full" style="overflow-y: auto; height: 500px;">
                <ul class="nav nav-tabs nav-justified nav_bg">
                    <li class="active"><a href="#Informacion_del_sistema" data-toggle="tab"><i
                                class="fa fa-laptop"></i>@lang('Información de Contratos- Cabildo Indígena de Guambia.') </a></li>
                </ul>
                    ...
                    <thead>
                        <tr>
                          <th>#</th>
                            ...
                            <th>Nombre</th>
                            <th>Apellido</th>
                            <th>Observaciones del contrato</th>
                            <th># N° Contrato SedCauca </th>
                            <th># N° Contrato E-Eduación </th>
                        ...
                        @foreach($datos as  $key=> $novedad)
                            <tr> 
                               <td>{{$key+1}}</td>
                                ...
                                <td>{{ $novedad->num_sedcauca }}</td>
                                <td>{{ $novedad->num_contrato}}</td>
                                <td>{{ $novedad->fecha_registro}}</td>
                             <!--   <td><button class="enlace" onclick="decargarActa({{ $novedad->id }});">Descargar
                                        Contrato</button></td>-->
                                        <td><button class="enlace" onclick="showFile('{{ $novedad->id}}')">Ver Contrato</button></td>
                                    <td><a href="/ver_info_contrato/{{($novedad->id) }}" class="btn btn-sm btn-default" style="background-color:#FFA400;border: 0px !important;color:white;"> <i
                                        class="fa fa-eye" aria-hidden="true"></i>
                                        Ver Información Contrato
                                    </a></td>
                                <!--    <td><a href="/Inicio/Talento-humano/{{($novedad->id) }}/actualizar" class="btn btn-sm btn-default" style="background-color:#FFA400;border: 0px !important;color:white;"> <i
                                        class="fa fa-eye" aria-hidden="true"></i>
                                        Editar Información Contrato
                                    </a></td>-->
...
<script>
    ...
    function showFile(id_novedad){
            $.ajax({
                ...
                url: '/verdocumento/' + id_novedad,
                type: "get",
                dataType: "html",
                contentType: false,
                processData: false
            }).done(function(res){
                docomento_pdf = JSON.parse(res).response.docomento_pdf
                window.open('/storage/'+docomento_pdf,'_blank');
            }).fail(function(res){
                console.log(res)
            });
        }0
